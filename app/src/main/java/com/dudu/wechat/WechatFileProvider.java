...
public class WechatFileProvider extends DocumentsProvider {
    
    //来自AndroidIDE的fileprovider方便我调试qwq
 ...
 */
  ...
  private static File getBaseDir() {
    return new File("/data/data/com.dudu.wechat/");
  }
  ...
  public Cursor queryRoots(String[] projection) {
    ...
    Log.d("FileProvider","queryRoots() before all add");
    row.add(Root.COLUMN_ROOT_ID, getDocIdForFile(BASE_DIR));
    Log.d("FileProvider","queryRoots() before all add, 1");
    row.add(Root.COLUMN_DOCUMENT_ID, getDocIdForFile(BASE_DIR));
    Log.d("FileProvider","queryRoots() before all add, 2");
    row.add(Root.COLUMN_SUMMARY, null);
    Log.d("FileProvider","queryRoots() before all add, 3");
    row.add(
        Root.COLUMN_FLAGS,
        Root.FLAG_SUPPORTS_CREATE | Root.FLAG_SUPPORTS_SEARCH | Root.FLAG_SUPPORTS_IS_CHILD);
    Log.d("FileProvider","queryRoots() before all add, 4");
    row.add(Root.COLUMN_TITLE, applicationName);
    Log.d("FileProvider","queryRoots() before all add, 5");
    row.add(Root.COLUMN_MIME_TYPES, ALL_MIME_TYPES);
    Log.d("FileProvider","queryRoots() before all add, 6");
    row.add(Root.COLUMN_AVAILABLE_BYTES, BASE_DIR.getFreeSpace());
    Log.d("FileProvider","queryRoots() before all add, 7");
  ...
  public Cursor querySearchDocuments(String rootId, String query, String[] projection)
    ...
    while (!pending.isEmpty() && result.getCount() < MAX_SEARCH_RESULTS) {
      ...
      try {
        isInsideHome = file.getCanonicalPath().startsWith(getBaseDir().getAbsolutePath());
      } catch (IOException e) {
        isInsideHome = true;
      }
      if (isInsideHome) {
